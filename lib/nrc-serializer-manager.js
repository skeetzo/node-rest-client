...
var SerializerManager = function() {
	...
	var _private = {
		"validate":function(serializer) {
			function validateProperties(_serializer, props) {
				var result = true;
				for (const propIndex in props) {
					var propType  = props[propIndex].split(":");
					if (!_serializer.hasOwnProperty([propType[0]]) || typeof _serializer[propType[0]] !== propType[1]) {
						result = false;
						break;
					}
				}
		...
		"encode": function (obj, parent) {
            ...
            for (propertyName in obj) 
                if (obj.hasOwnProperty(propertyName)) {
	                ...
	                tokens.push((propertyValue !== null && typeof propertyValue === "object") ? serialize(propertyValue, parsedProperty) : encodeURIComponent(parsedProperty) + "=" + encodeURIComponent(propertyValue));
                }
            return tokens.join("&");
	    },	
		"serialize": function(data, nrcEventEmitter, serializedCallback) {
