...
exports.Client = function (options) {
    ...
    const parserManager = require("./nrc-parser-manager")();
    const serializerManager = require("./nrc-serializer-manager")();
    // const validatorManager = require("./nrc-validator-manager")();
    // connection manager
    const connectManager = new ConnectManager(this, parserManager);
    // io facade to parsers and serailiazers
    // const ioFacade = function(parserManager, serializerManager, validatorManager) {
    const ioFacade = function(parserManager, serializerManager) {
            ...
            // result={"parsers":{}, "serializers":{}, "validators": {}};
            ...
            // validators facade
            // result.serializers.add = errorContext(validatorManager.add);
            // result.serializers.remove = errorContext(validatorManager.remove);
            // result.serializers.find = errorContext(validatorManager.find);
            // result.serializers.getAll = errorContext(validatorManager.getAll);
            // result.serializers.getDefault = errorContext(validatorManager.getDefault);
            // result.serializers.clean = errorContext(validatorManager.clean);

            return result;
    
    }(parserManager, serializerManager);
    // }(parserManager, serializerManager, validatorManager);
    ...
    this.useProxy = (this.options.proxy || false) ? true : false,
    ...
    // namespaces for methods, parsers, serializers
    this.methods = {};
    this.parsers = {};
    this.serializers = {};
    // this.validators = {};
        ...
        createConnectOptions: function(connectURL, connectMethod) {
            ...
            if (self.useProxy) {
                result.proxy = {
                    ...
                    // createProxyHeaders add correct headers depending on proxy connection type
    ...
    this.registerMethod = function(name, url, method) {
        // create method in method registry with preconfigured REST invocation method
    ...
    // this.validators = ioFacade.validators;
...
// var ConnectManager = function(client, parserManager, validatorManager) {
var ConnectManager = function(client, parserManager) {
    ...
    this.handleResponse = function(res, data, callback) {
        // find valid parser to be used with response content type, first one found
        parserManager.get(res).parse(data, clientEmitterWrapper(client), function(parsedData) {
            callback(parsedData, res);
        // validatorManager.get(res).validate(parsedData, clientEmitterWrapper(client), function(validatedDate) {
            // callback(validatedData, res);
        // });
        });
    };
