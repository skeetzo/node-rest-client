...
var ParserManager = function() {
	...
	var _private = {
		"validate": function (parser) {
			function validateProperties(_parser, props) {
				var result = true;
				for (const propIndex in props) {
					var propType  = props[propIndex].split(":");
					if (!_parser.hasOwnProperty([propType[0]]) || typeof _parser[propType[0]] !== propType[1]){
						result = false;
						break;
					}
				}
	...
	this.remove = function(parserName) {
		const result = registry[parserName];
		if (!result)
	...
	this.get = function(response) {
		var result = null;
		for (const parserName in registry)
...
module.exports = function() {
	...
	//add default validation managers
